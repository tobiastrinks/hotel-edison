...
function elements_relative(){
	...
	if(glob_cms_device == "desktop"){
		...
		$("dl").css("width", ($("body").width()*0.4)+"px");
		$(".short_description").css("width", ($("body").width()*0.4)+"px");
		
		var breite = $("body").width();
		var scroll_pos = $(window).scrollTop();
		var form_height = $("#buchungsanfrage").offset();
		var form_height_bottom = form_height.top + $("#form_anfrage").height();
		...
		zimmerbilder.css("width", breite*0.45).css("right", breite*0.1);
		
		button_left.css("left", breite*0.45-12);
		button_right.css("left", breite*0.9-70);
		
		
		if(scroll_pos + 200 < form_height.top){
		...
		if(scroll_pos +200+bildhoehe > form_height_bottom){
			
			button_left.css("top", form_height_bottom-bildhoehe-form_height.top+(breite*0.45*0.70)*0.5 - 20);					
			button_right.css("top", form_height_bottom-bildhoehe-form_height.top+(breite*0.45*0.70)*0.5 - 20);
			
			zimmerbilder.css("top", form_height_bottom-bildhoehe-form_height.top);
		
		}
		
		if(scroll_pos + 200 > form_height.top && scroll_pos +200+bildhoehe < form_height_bottom){
		
			var top = scroll_pos-form_height.top+200;
...
function anfrage_oeffnen(){
	...
	anfrage_head.css("borderRadius", "0 10px 0 0").css("display", "block");
...
function weiter(){
	...
	if(this_element == "reisezeitraum"){
		...
		if($("dd[data-info='reisezeitraum'] #anreise").val() != 0){
			...
			if($('input[type=radio][name=buchungstyp]:checked').val() == "arr")
				...
				if($("dd[data-info='reisezeitraum'] #abreise1").val() != 0)
					weiter=true;
			}
	...
	if(weiter == true){
		...
		next_element.css("borderRadius", "0 10px 0 0");
		next_element.css("borderBottom", "1px solid #d2c57f");
...
function zusammenfassung_aktualisieren(){
	...
	var reiseteilnehmer_k1		= $('select[id="kin1"]').val();
	var reiseteilnehmer_k1_text	= $("label[for='kin1']").text();
	
	var reiseteilnehmer_k2		= $('select[id="kin2"]').val();
	var reiseteilnehmer_k2_text	= $("label[for='kin2']").text();
	
	var reiseteilnehmer_k3		= $('select[id="kin3"]').val();
	var reiseteilnehmer_k3_text	= $("label[for='kin3']").text();
	...
	if(glob_cms_lang == "eng")
		$("#zsf_reisezeitraum").text("from "+reisezeitraum_von+" to "+reisezeitraum_bis);
	
	$("#zsf_zimmertyp").text(zimmertyp_text);
	
	
	$("#zsf_reiseteilnehmer_erw").css("display", "block");
	$("#zsf_reiseteilnehmer_erw").text(reiseteilnehmer_e + " " + reiseteilnehmer_e_text);
	
	if(reiseteilnehmer_k1 != "0"){
		$("#zsf_reiseteilnehmer_kin1").text(reiseteilnehmer_k1 + " " + reiseteilnehmer_k1_text);
		$("#zsf_reiseteilnehmer_kin1").css("display", "block");
	}
	else{
		$("#zsf_reiseteilnehmer_kin1").css("display", "none");
	}
	
	if(reiseteilnehmer_k2 != "0"){
		$("#zsf_reiseteilnehmer_kin2").text(reiseteilnehmer_k2 + " " + reiseteilnehmer_k2_text);
		$("#zsf_reiseteilnehmer_kin2").css("display", "block");
	}
	else{
		$("#zsf_reiseteilnehmer_kin2").css("display", "none");
	}
	
	if(reiseteilnehmer_k3 != "0"){
		$("#zsf_reiseteilnehmer_kin3").text(reiseteilnehmer_k3 + " " + reiseteilnehmer_k3_text);
		$("#zsf_reiseteilnehmer_kin3").css("display", "block");
	}
	else{
		$("#zsf_reiseteilnehmer_kin3").css("display", "none");
	}
...
$(document).ready(function() {
	...
	$(".feiertag_select").click(function(){
		...
		if(glob_cms_lang == "eng"){
			datum = datum.replace(datum.slice(datum.indexOf(" to "), datum.length) , "");
		}
	...
	$('input[type=radio][name=zimmertyp]').change(function() {
		...
		if(glob_cms_device == "desktop"){
			...
			$("#button_left").hide(300);
			$("#button_right").hide(300);
